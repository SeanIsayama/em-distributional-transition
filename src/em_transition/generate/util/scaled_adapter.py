...
def scaled_B(model, layer_idx: int, scale: float) -> Generator:
    ...
    Writes to B.data in place so the parameter reference stays live across
    checkpoint swaps; the original is always restored on exit, even if the
    body raises. Nesting is not supported.
